...
func _ready() -> void:
	...
	for ab in GameData.player_1_abilities: # Ab stands for ability
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("player_1_ability_cycle"):
		...
		if ability_selected > 2:
			ability_selected = 0
		info_overlay.update_ability_icons(1, GameData.player_1_abilities, ability_selected)
...
func apply_dmg(damage):
	GameData.player_1_health -= abs(round(damage))
	if GameData.player_1_health <= 0:
		print("Player 1 has died! Player 2 wins!")
		queue_free() # "Kills" the player if there health is gone
	print(GameData.player_1_health, " Health left for player 1")
